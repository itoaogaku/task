...
### ボード（人ごとの区画）でシートが増える
URLに `?board=名前` を付けると、その名前ごとに**別のデータ区画**になる（詳細は §11）。
このとき上記4シートが `Tasks__名前 / Archive__名前 / Memo__名前 / Assignees__名前` として自動生成される。
`?board=` なし（既定ボード）は従来どおり `Tasks / Archive / Memo / Assignees` を使う。
...
## 11. ボード（人ごとに別々の内容を見せる）

Claude Code を使えない相手にも、**1つのアプリ・1つのバックエンドのまま**、
人ごとに別のタスク/保管/メモ/確認先を持たせられる仕組み。相手側の Google 設定は不要。

### 使い方（共有する側）
- 相手に渡すURLの末尾に `?board=名前` を付けるだけ。
  - 例：`https://<アプリのURL>/?board=tanaka` … 田中さん専用
  - 例：`https://<アプリのURL>/?board=sato` … 佐藤さん専用
  - `?board=` なしのURL … 既定（共有）ボード（従来のデータ）
- 名前は半角英字が無難（日本語も可）。`[ ] * / \ ? :` は使えないので自動で除去される。上限40文字。
- 相手はそのURLを**ホーム画面に追加**すれば全画面PWAとして使える（§手順は本文他所と同じ）。

### 仕組み
- フロント（`app.js`）：起動時に `?board=` を読み、端末に記憶（`localStorage: task_board_v1`）。
  以降 `?board=` なしで開いても同じボードを表示する。
  端末内キャッシュ/未送信キュー/確認先もボードごとに分離（キー末尾に `__<board>`）。
  すべてのAPI呼び出しに `board` を付与。
- バックエンド（`Code.gs`）：`CURRENT_BOARD` をリクエストごとに `params.board` から確定し、
  `sheetName_()` で各シート名に `__<board>` を付ける。`runRecurring`（毎日のトリガー）は
  `Archive__*` を含む全ボードの保管シートを巡回して繰り返しタスク化する。
- **GAS再デプロイが必要**（§8）。既定ボードのデータは影響を受けない。

### 注意（セキュリティ）
- URL（トークン＋board）を知っている人は、そのボードのデータを見る・編集できる簡易的な仕組み。
  **社内の信頼できる相手にのみ**共有すること。board名は推測されにくい文字列にしてもよい。
- ボードは相手が初めてそのURLで何か操作した時点で自動作成される（事前準備不要）。
