...
- **保管**：日付＋繰り返し（登録/毎月/毎年）で記録。毎月・毎年はその日付になると自動でタスク化（保管データは消えない）。
  - 発火は「その日ちょうど」だけでなく**期日を過ぎた分も当該期間内で一度だけ拾う**（取りこぼし防止）。判定は毎月＝同一年月／毎年＝同一年（`lastFired`列で管理）。月末補正あり。
  - **アプリを開くたび**（`listAll_`）にもチェックが走るので、毎日1回のトリガーが未設定でも自動タスク化が機能する（トリガーも併用推奨）。
- **メモ**：自由記入のメモ帳
